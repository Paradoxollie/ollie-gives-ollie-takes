...
function listAvailableRewardIds(
  ...
  return ADVENTURE_REWARD_POOLS[rarity].filter((archetypeId) => {
    ...
    return !usedArchetypeIds.has(archetypeId) && !excludedBaseIds.has(baseId);
  });
}
...
function pickRewardRarity(
  ...
  return pickWeightedRarity(
    [
      {
        rarity: "common",
        weight: listAvailableRewardIds("common", usedArchetypeIds, excludedBaseIds).length > 0 ? baseWeights.common : 0,
      },
      {
        rarity: "uncommon",
        weight:
          offeredByRarity.uncommon >= ADVENTURE_REWARD_CONFIG.maxUncommonOffersPerRun ||
          listAvailableRewardIds("uncommon", usedArchetypeIds, excludedBaseIds).length === 0
            ? 0
      ...
      },
      {
        ...
        weight:
          ...
          listAvailableRewardIds("rare", usedArchetypeIds, excludedBaseIds).length === 0
            ? 0
            : baseWeights.rare,
      },
    ],
...
function pickRewardOption(
  ...
  const availableRewardIds = listAvailableRewardIds(rarity, usedArchetypeIds, excludedBaseIds);
...
export function createAdventureRewardOffer(options: {
  ...
  const excludedBaseIds = new Set(
    options.playerDeck?.cards.map((entry) => entry.card.baseArchetypeId ?? entry.card.id) ?? [],
  );
  const forcedUncommonCount = Math.min(
    getGuaranteedUncommonCount(options.progress),
    listAvailableRewardIds("uncommon", usedArchetypeIds, excludedBaseIds).length,
  );
  ...
  for (let slotIndex = 0; slotIndex < ADVENTURE_REWARD_CONFIG.offerSize; slotIndex += 1) {
    ...
    const pickedOption = pickRewardOption(rarity, usedArchetypeIds, excludedBaseIds, nextSeed, offerIndex, slotIndex);
...
export function createAdventureStealRewardOffer(options: {
  ...
  const uniqueEnemyCards = new Map<string, CardArchetype>();

  for (const card of options.enemyCards) {
    const baseId = card.baseArchetypeId ?? card.id;
    if (ownedBaseIds.has(baseId) || uniqueEnemyCards.has(baseId)) {
      continue;
    }

    uniqueEnemyCards.set(baseId, card);
  }

  const candidates = [...uniqueEnemyCards.values()];
  ...
  for (let index = 0; index < optionCount; index += 1) {
    ...
    pickedOptions.push({
      rewardId: `steal-${options.sourceNodeId}-slot-${index + 1}-${picked.item.id}`,
      archetypeId: picked.item.id,
      rarity: picked.item.rarity,
    });
    remaining.splice(remaining.findIndex((card) => card.id === picked.item.id), 1);
  }
