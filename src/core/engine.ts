...
function drawCardsForTurn(state: MatchState, playerId: PlayerId): {
  ...
  let cardsToDraw = state.config.cardsPerTurn + extraDraw;
...
export function applyMove(state: MatchState, move: MoveInput): MatchState {
  ...
  if (roundTriggered) {
    const roundEnd = resolveRoundEnd(state, nextBoard, state.champions, nextEnemyPowerState);
...
export function previewMove(state: MatchState, move: MoveInput): PreviewMove {
  ...
  const roundEnd = roundEnds ? resolveRoundEnd(previewState, resolution.board, state.champions, previewEnemyPowerState) : null;
  ...
  return {
    ...
    resultingWinner: roundEnd?.result?.winner ?? null,
  };
}
