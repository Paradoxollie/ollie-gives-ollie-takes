...
def split_row(row: Image.Image, row_name: str, frame_count: int) -> list[PreparedFrame]:
    transparent = remove_tiny_components(remove_connected_chroma(row), min_pixels=18)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build transparent game sprite strips from hatch-pet generated Roncy cat rows.")
    parser.add_argument("--run-dir", type=Path, default=Path("work/runs/roncy-hatch-game"))
    parser.add_argument("--out-dir", type=Path, default=Path("public/images/units/cat"))
    parser.add_argument("--audit-dir", type=Path, default=Path("output/playwright/cat-hatch-frame-audit"))
    ...
    print(f"Wrote hatch-pet cat assets to {args.out_dir.resolve()}")
