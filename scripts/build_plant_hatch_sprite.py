...
def is_chroma_background(pixel: tuple[int, int, int, int]) -> bool:
    ...
    return r >= 170 and b >= 150 and g <= 95 and r - g >= 90 and b - g >= 80
...
def clear_chroma_fringe(image: Image.Image) -> None:
    ...
    for y in range(1, height - 1):
        for x in range(1, width - 1):
            ...
            if touches_transparency and r >= 150 and b >= 135 and g <= 120 and r - g >= 60 and b - g >= 50:
                clear.append((x, y))
...
def slot_for_bbox(bbox: tuple[int, int, int, int], width: int, frame_count: int) -> int:
    x0, _, x1, _ = bbox
    center_x = (x0 + x1) / 2
    return min(frame_count - 1, max(0, int(center_x / width * frame_count)))
...
def split_row(row: Image.Image, row_name: str, frame_count: int) -> list[PreparedFrame]:
    ...
    grouped: dict[int, list[tuple[int, tuple[int, int, int, int], set[tuple[int, int]]]]] = defaultdict(list)
    for component in components:
        grouped[slot_for_bbox(component[1], transparent.width, frame_count)].append(component)

    row_pixels = transparent.load()
    frames: list[PreparedFrame] = []
    for index in range(frame_count):
        frame_components = grouped.get(index, [])
        if not frame_components:
            raise RuntimeError(f"{row_name} frame {index + 1} has no visible component")

        points: set[tuple[int, int]] = set()
        for _, _, component_points in frame_components:
            points.update(component_points)

        xs = [point[0] for point in points]
        ys = [point[1] for point in points]
        bbox = expand_box((min(xs), min(ys), max(xs) + 1, max(ys) + 1), transparent.width, transparent.height, 8)
        x0, y0, x1, y1 = bbox
        ...
        frames.append(PreparedFrame(f"{row_name}-{index + 1}", isolated, (0, 0, isolated.width, isolated.height)))
...
def normalize_sequence(
    ...
    *,
    ...
    for frame in frames:
        ...
        paste_x = round((FRAME_SIZE - resized.width) / 2)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build transparent game sprite strips from hatch-pet generated Floramie rows.")
    parser.add_argument("--run-dir", type=Path, default=Path("work/runs/floramie-hatch-game"))
    parser.add_argument("--out-dir", type=Path, default=Path("public/images/units/plant"))
    parser.add_argument("--audit-dir", type=Path, default=Path("output/playwright/plant-hatch-frame-audit"))
    ...
    if bbox is not None:
        ...
        scale = min(438 / base_crop.width, 430 / base_crop.height)
    ...
    print(f"Wrote hatch-pet plant assets to {args.out_dir.resolve()}")
